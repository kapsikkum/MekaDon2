...
from meka.core.models import CommandErrorNotification
...
class VoiceState:
    ...
    def play_next_song(self, error=None):
        if error:
            raise CommandErrorNotification(str(error))
...
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    async def create_source(cls, ctx, search: str, *, loop=None):
        ...
        if data is None:
            raise CommandErrorNotification(
                "Couldn't find anything that matches `%s`" % search
            )
        ...
        if processed_info is None:
            raise CommandErrorNotification("Couldn't fetch `%s`" % webpage_url)
    ...
    def parse_duration(duration):
        ...
        duration = []
        if days > 0:
            duration.append("%s days" % days)
        if hours > 0:
            duration.append("%s hours" % hours)
        if minutes > 0:
...
class Cog(commands.Cog, name="Music Commands"):
